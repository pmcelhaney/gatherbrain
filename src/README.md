...
Enum registry loader. It merges `default-config/enums.json` with workspace-local `.gatherbrain/enums.json`. Enums are used for command argument validation and autocomplete, for example the `factType` enum used by metadata shorthand.
