...
test('formats display times without military hours or leading zeroes', () => {
  ...
  assert.equal(formatDisplayClockTime(8 * 60, { padHour: true }), ' 8:00');
...
test('renders planner timeline blocks with durations', async () => {
  ...
  try {
    ...
    assert.deepEqual(lines, [
      '   8:00  ○  free · 1h',
      '   9:00  ●  /arb-prep · 2h',
      '  11:00  ●  /arb/meetings/2026-06-29 · 30m',
      '  11:30  ●  /arb-prep · 30m',
      '  12:00  ○  free · 2h',
      '   2:00  ●  /team-meeting/2026-06-29 · 1h',
      '   3:00  ○  free · 3h'
    ]);
  } finally {
    await rm(rootDirectory, { recursive: true, force: true });
...
test('expands planner rows when timeboxes fall outside the workday', () => {
  ...
  assert.equal(lines[0], '   7:00  ●  /early · 30m');
  assert.equal(lines[1], '   7:30  ○  free · 11h');
  assert.equal(lines.at(-1), '   6:30  ●  /late · 30m');
});
